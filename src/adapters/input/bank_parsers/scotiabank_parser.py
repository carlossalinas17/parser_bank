...
class ScotiabankParser(BankParser):
    ...
    def _extraer_movimientos(
        ...
        print("\n" + "=" * 80)
        print("INICIANDO PROCESAMIENTO SCOTIABANK")
        print("=" * 80)
        ...
        for page in pages:
            ...
            print(f"\n--- Procesando página con {len(lineas)} líneas ---")
            ...
            while i < len(lineas):
                ...
                if self._es_inicio_seccion(linea):
                    en_seccion = True
                    print(f"[LÍNEA {i}] Inicio de sección detectado: '{linea}'")
                    i += 1
                    continue
                ...
                # Saltar encabezados repetidos y líneas no relevantes
                if self._es_linea_ignorable(linea):
                    print(f"[LÍNEA {i}] Línea ignorada: '{linea}'")
                    i += 1
                    continue
                ...
                print(f"\n[LÍNEA {i}] ENCONTRADO MOVIMIENTO: '{linea}'")
                ...
                while j < len(lineas) and lineas_agregadas < self._MAX_CONCEPT_LINES:
                    ...
                    # Si encontramos otra fecha → nuevo movimiento, detener
                    if self._DATE_PATTERN.match(siguiente):
                        print(f"[LÍNEA {j}] Otra fecha encontrada, deteniendo acumulación")
                        break
                    ...
                    # Encabezado repetido → detener
                    if self._es_linea_ignorable(siguiente):
                        print(f"[LÍNEA {j}] Encabezado encontrado, deteniendo: '{siguiente}'")
                        break

                    concepto_lines.append(siguiente)
                    lineas_agregadas += 1
                    print(f"[LÍNEA {j}] Agregada al concepto: '{siguiente}'")
                    j += 1
    ...
    def _procesar_movimiento(
        ...
        print(f"  Montos encontrados: {montos_raw}")
        print(f"  Montos válidos: {montos_validos}")

        if len(montos_validos) < 2:
            print("  ERROR: No hay suficientes montos válidos (se necesitan al menos 2)")
            return None

        # El primer monto válido es el monto de la operación
        monto_operacion = montos_validos[0]
        print(f"  Monto de operación: {monto_operacion}")

        # Clasificar por keywords
        concepto_para_tipo = texto_completo
        concepto_para_tipo = re.sub(r"^\d{2}\s+[A-Z]{3}\s+", "", concepto_para_tipo).strip()
        tipo = self._detectar_tipo(concepto_para_tipo)
        print(f"  Tipo detectado: {tipo}")

        deposito = monto_operacion if tipo == "deposito" else Decimal("0")
        retiro = monto_operacion if tipo == "retiro" else Decimal("0")

        # Extraer referencia (10+ dígitos en el concepto)
        referencia = ""
        ref_match = self._REFERENCE_PATTERN.search(texto_completo)
        if ref_match:
            referencia = ref_match.group(1)
            print(f"  Referencia extraída: {referencia}")

        # Limpiar concepto: mantener TODO excepto los montos en formato $X,XXX.XX
        # Pero mantener porcentajes, números de operación, referencias, etc.

        # Primero, eliminar los montos específicos que encontramos
        concepto_limpio = texto_completo
        for monto_str in montos_raw:
            # Buscar el monto con el signo $ incluido
            monto_con_dolar = f"${monto_str}"
            concepto_limpio = concepto_limpio.replace(monto_con_dolar, "")

        # También eliminar cualquier $ suelto que haya quedado
        concepto_limpio = re.sub(r"\s+\$\s+", " ", concepto_limpio)

        # Eliminar la fecha al inicio si está
        concepto_limpio = re.sub(r"^\d{2}\s+[A-Z]{3}\s+", "", concepto_limpio).strip()

        # Limpiar espacios múltiples
        concepto_limpio = " ".join(concepto_limpio.split()).strip()

        print(f"  Concepto final: '{concepto_limpio[:200]}...'")
    ...
    def _detectar_tipo(self, concepto: str) -> str:
        ...
        for keyword in self._WITHDRAWAL_KEYWORDS:
            if keyword in concepto_upper:
                print(f"    Keyword de retiro encontrada: '{keyword}'")
                return "retiro"

        # Luego evaluar depósitos
        for keyword in self._DEPOSIT_KEYWORDS:
            if keyword in concepto_upper:
                print(f"    Keyword de depósito encontrada: '{keyword}'")
                return "deposito"
